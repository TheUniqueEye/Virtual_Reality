...
//
// [INNER] Version 3: reconstructed with Karl's fragmented-image example
//
...
struct MyApp : App {
    ...
    MyApp() {
      ...
      samplePlayer.load(fullPathOrDie("heart_beat.wav").c_str());
      initAudio();
      gam::Sync::master().spu(audioIO().fps()); ///
      initWindow();
      nav().pos(0,0,25); /// 设定一个初始的观看的位置
