...
// [INNER] Version 6 : multi images added, sound fixed, scene rearranged
//
...
struct MyApp : App, AlloSphereAudioSpatializer, InterfaceServerClient {
  cuttlebone::Maker<State, 9000> maker;
    ...
    MyApp()
      ...
      fragmentSoundSamples.load(fullPathOrDie("inner_1.wav").c_str()); // XXX
  ...
  virtual void onAnimate(double dt) {
    ...
    float a = 2.5;
    fragmentedImage[0].setPosition(Vec3f(0,0,-5*a)); // XXX XXX how to set the position
  ...
  virtual void onSound(AudioIOData& io) {
    ...
    //
    listener()->pos(0, 0, 0);
    //listener()->pose(Pose(Vec3f(0, 0, 0), Quatf()));
    scene()->render(io);
  }
